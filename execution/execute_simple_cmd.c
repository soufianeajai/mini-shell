...
//         return (126);
//     }
//     else if(!S_ISREG(path_stat.st_mode) && (ft_strchr(cmd,'/') || (!ft_strchr(cmd,'/') && cmd[0] == '/')))
//     {
        
//         if (flag)
//         {
//             if (cmd[ft_strlen(cmd) - 1] == '/' && S_ISDIR(path_stat.st_mode))
//             {
//                 ft_error(cmd, "Not a directory");
//                 return (126);
//             }
//             else
...
int execute_simple_cmd(t_env **env, t_cmd_node *cmd)
{
    ...
    {
        printf("\n%s\n path %s %s",strerror(errno), path_cmd,cmd->arguments[0]);
        if (errno == ENOTDIR)
            return(ft_error(cmd->executable, "Not a directory"));
